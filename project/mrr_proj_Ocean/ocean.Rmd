...
coef_bats_linear <- list()
coef_bats_forward <- list()
coef_bats_ridge <- list()
coef_bats_lasso <- list()
coef_bats_elastic_net <- list()
...
for (i in 1:17) {
  ...
  coef_bats_linear <- append(coef_bats_linear, coef(reg_linear))
  ...
  # add 0 to removed variables for comparison with other methods
  coef_forward_with_zero <- coef(reg_forward)
  # 7 = target variable
  for (var in colnames(train_data_CHLi[,-7])) {
    if (!(var %in% names(coef(reg_forward)))) {
      coef_forward_with_zero <- c(coef_forward_with_zero, 0)
      # update vector name by adding the removed variable name
    }
  }
  
  coef_bats_forward <- append(coef_bats_forward, coef(reg_forward))
  ...
  coef_bats_ridge <- append(coef_bats_ridge, coef(reg_ridge))
  ...
  coef_bats_lasso <- append(coef_bats_lasso, coef(reg_lasso))
  ...
  for (a in 0:10) {
    ...
    all_elastic_net_models <- append(all_elastic_net_models, reg_elastic_net)
  ...
  coef_bats_elastic_net <- append(coef_bats_elastic_net, coef(all_elastic_net_models[[min_error_index]]))
