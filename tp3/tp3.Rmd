---
title: "Practical session 3 - Modèles de régression régularisée"
...
With Ridge regression, we start by finding the best value of $\lambda$, by performing k-fold cross-validation ($k=10$ in this case) and find the value of $\lambda$ that minimizes the cross-validated error.
```{r}
library(glmnet)
...
After obtaining the value of $\lambda$, we then use it to compute the Ridge regression.
```{r}
...
Similar to Ridge regression, with Lasso regression, we begin by selecting the best value of $\lambda$ by performing k-fold cross-validation.
```{r}
library(glmnet)
...
We now compute the Lasso regression with the obtained value of $\lambda$.
```{r}
...
```
We can see that the model of `r switch(which.min(c(error_logit, error_forward, error_ridge, error_lasso)), "full logistic regression", "forward method", "Ridge regression", "Lasso regression")` makes the least errors. We can also say that with a data set of 85 observations (the 20% training data set), the model of full logistic regression, forward method, Ridge regression and Lasso regression make `r error_logit`, `r error_forward`, `r error_ridge` and `r error_lasso` mistakes, respectively.
...
```
Based on the comparison above, we can see that the model of `r switch(which.max(c(perf_logit$accuracy, perf_forward$accuracy, perf_ridge$accuracy, perf_lasso$accuracy)), "full logistic regression", "forward method", "Ridge regression", "Lasso regression")` is the best in terms of accuracy. It is also worth noting that the performances of the 4 models are not too different from each other.
