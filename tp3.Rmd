...
Firstly, we load the data into a data frame. We then convert the values of `Y` into binary values (`YBin`) using the median of `Y` as the threshold:
...
Now, we randomly split the data into 2 sets: training data set (80% of the data) to calibrate the models, and testing data set (the remaining 20% of the data) to evaluate the models after the calibration.
```{r}
...
Given the observations and predictions, the function below is used to compute the confusion matrix of a model along with value of the accuracy, the precision, the recall and the specificity.
```{r}
...
Here, we are using the 80%-20% splitted data above to train and test the logistic model.
```{r}
...
for (i in 1:5) {
  ...
  #TP <- sum((tab_test$YBin == 1) + (YBin_predict_MAP == 1) == 2)
  #TN <- sum((tab_test$YBin == 0) + (YBin_predict_MAP == 0) == 2)
  #FP <- sum((tab_test$YBin == 0) + (YBin_predict_MAP == 1) == 2)
  #FN <- sum((tab_test$YBin == 1) + (YBin_predict_MAP == 0) == 2)
